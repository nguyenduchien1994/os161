...
int linkedlist_test_runtests(int nargs, char **args)
{
    ...
    if (testnum == 0) {
      ...
      thread_fork("Test 2 Thread",
		  NULL,
		  linkedlist_test_adder,
		  list,
		  1);

      // Let new thread process and before adding.
      thread_yield();  

      // Run same function in main thread.
      linkedlist_test_adder(list, 2);
      ...
      // Let main thread proceed to after empty check, and then swap
      // back to the new thread.  
      linkedlist_test_insert(list, 1); 
      ...
      // Remove with main thread.
      linkedlist_test_remove(list, 2); 
      ...
      // Insert with main thread.
      linkedlist_test_insert(list,2);
      ...
      // Simulatenous insert corrupts list -> length, very similar to
      // example in class.
